...
def celebA_MAML(num_tasks, ways, shots, iterations, batch_size, global_labels):
    data_plot, accuracy = MAML_celeb.main(tasks=num_tasks, ways=ways * num_tasks, meta_batch_size=batch_size,
                                          shots=shots, num_iterations=iterations, global_labels=global_labels)
    with open(
            f"./results/MAML/celebA/{num_tasks}_{ways}_{shots}_{iterations}_{batch_size}_{str(global_labels)}",
            "w") as my_csv:
    ...
    # plt.plot(iteration_list, val_acc, color='goldenrod', label='Validation Accuracy')
    # plt.title(
...
def tasksets_MAML(taskset, num_tasks, ways, shots, iterations, batch_size):
    ...
    with open(
            f"./results/MAML/{taskset}/{num_tasks}_{ways}_{shots}_{iterations}_{batch_size}", "w") as my_csv:
        csvWriter = csv.writer(my_csv, delimiter=',')
        csvWriter.writerows(data_plot)
...
if __name__ == '__main__':
    ...
    celebA_MAML(num_tasks=10,
                ...
                shots=5,
                iterations=500,
                batch_size=256,  # 32 wasmt big enough
                global_labels=True)
